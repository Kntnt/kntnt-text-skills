...
- **`/eval` capture command — turn a real session into an eval case.** A new maintainer-only command under `evals/` (`evals/eval/SKILL.md` plus the deterministic core `evals/capture.py`) reads a satisfied `/write`, `/edit` or `/redline` session — the maintainer's running feedback as the primary fault source, corroborated by the before/after diff — and *proposes* a candidate case without writing anything: a brief (for `/write`) or input text (for `/edit` / `/redline`) plus dimension-tagged assertions, each traced to the feedback it came from, shaped per skill (deterministic catch/apply for `/edit` / `/redline`, negative/structural for `/write`) with names anonymised. The maintainer ratifies line by line and decides commit-or-skip; only on commit is the case appended to `evals/evals.json` in object form and the per-skill files regenerated (flattened to plain strings, reusing the issue #44 regeneration). The dimension on each assertion comes from `evals/classify_dimensions.py`, so a captured case scores on the same footing as a hand-authored one. Lives entirely under `evals/`; the skills under `lib/` are untouched.
- **Register scored as a separate eval dimension.** Every assertion in `evals/evals.json` now carries a `dimension` (`protocol`, `mechanics` or `register`), and `_aggregate.py` reports two with-skill numbers instead of one pooled rate: a protocol/mechanics sub-score (the hard release gate, any regression blocks) and a register sub-score reported with its assertion count (a tracked improvement target, only a regression against the prior register baseline is flagged). The grader prompt is unchanged – `_setup.py` extracts the assertion text into a flat list and carries the dimensions through in a parallel `assertion_dimensions` list – and the per-skill `evals.json` exports stay flat plain-string lists. `evals/classify_dimensions.py` documents and reproduces the classification; `evals/baseline.md` splits its release-gate wording accordingly.
...
- **`/eval` capture proposes faithful, robust candidates.** Five derivation faults in `evals/capture.py` are fixed so the proposed case reflects the maintainer's feedback rather than artefacts of the reshaping. An `/edit` or `/redline` assertion's **dimension** is now classified from the source remark together with the shaped text, not from the term-stripped reshape, so an explicit anglicism the maintainer named no longer mis-tags as `mechanics` when its fault word is absent from the register marker list. The **anonymiser** stopword set now holds genuine prose words only — the planted fixture first names are gone — so a single standalone first name (`Erik`) scrubs instead of leaking, and the `/eval` ratification step now tells the maintainer to eyeball the scrubbed fixture for residual names, since the heuristic is not robust named-entity recognition. **Correction-target extraction** requires an *emphasised* target after the connective, so an incidental "to" or "become" in prose ("needs to go") no longer fabricates a correction the maintainer never named. A **multi-fault remark** now seeds one assertion per distinct emphasised fault instead of keeping only the first, with the maintainer's listed correction alternatives excluded from the fault set. And the `input_text` reconstructed from the before/after diff no longer carries a stray leading space on every line. The capture and register-dimension tests stay green and the audit gate is unaffected.
- **`/eval` capture no longer leaks names into committed assertions, loses correction targets, or truncates quoted remarks.** Three further faithfulness gaps in `evals/capture.py` are closed. The **assertion text** is now anonymised, not just the brief and input text: a name the maintainer cited inside a fault remark (*Marx's teorier*, an *Acme delivers value* phrase) used to survive verbatim into the committed `evals.json`, the one object form `commit` actually keeps; the proposal-only `source` trace stays verbatim for provenance, and the SKILL.md ratification note now tells the maintainer to eyeball each assertion's text. The **correction-connective allowlist** gains the natural-English *change to* and *should be*, and the fault-to-target pairing tolerates ordinary words between the fault and the connective, so a redline remark like "*utilise* is jargon, change to *use*" pairs the fault with its target instead of proposing the correction word *use* as its own fault. The **quoted-remark regex** is de-duplicated to one opening and one closing quote class and made greedy, so a feedback bullet whose remark contains an inner smart-quoted span is captured whole rather than truncated at the first inner quote. The capture and register-dimension tests stay green and the audit gate is unaffected.
- **Register dimension tags corrected to match editorial judgement.** Twenty-six assertions in `evals/evals.json` carried the wrong dimension because the migration committed raw classifier output without the promised editorial pass: six bare-phrased AI-tell / opening-cliché assertions sat in `mechanics` while their near-identical siblings were `register` (under-counting register and wrongly pooling them into the hard gate); two fast-path short-text restraint checks sat in `register` instead of `protocol`; nine procedural assertions had dropped to the `mechanics` fallback; the fast-path negative-control discriminator and the flow-load consequence in the structured-article case sat in `mechanics` rather than `protocol`; and the proofread-scope, output-unchanged and no-silent-rewrite restraint checks — the same negative-control discipline as their `protocol`-tagged siblings — had likewise dropped to the `mechanics` fallback (a convention-led twin that merely appends the restraint clause is held in `mechanics` by a mechanics override). `classify_dimensions.py` now encodes the corrected rules, and the committed suite is verified to match its output (a new test fails on any divergence). The `classify` and README claim of a hand-review is corrected to "the suite is kept in sync with the classifier".
- **`_aggregate.py` no longer drops unmatched graded assertions silently.** The dimension join is by verbatim text; when the graded text and the suite text drift (for example a quote-style change), the affected assertions used to vanish from the gate count with no notice. The aggregator now counts and reports them (to stderr and in the report) so an under-count is visible. A new `--workspace` flag lets the dimension-tagged suite in one checkout score the real, untracked grading outputs in another in a single command, so the register sub-score can be proven on the real outputs without splicing two trees by hand.

### Changed

- **`/write` and `/edit` now default to one adversarial review iteration.** The polish step (`/write` Phase 4, `/edit` Phase 3) previously defaulted to `--max-iterations=0` – the main agent applied its own redline findings to its own draft in the same context, a check too weak to catch register faults, so translated-reading prose passed every objective gate and shipped. The default is now `--max-iterations=1`: one subagent round briefed adversarially. `subagent.md` recasts the subagent from a collegial editor who *wants the text to succeed* into an adversarial register reviewer who *assumes the text is AI-generated or translated* and hunts every sentence that still reads that way; the adversarial stance is toward the text, not punitive toward the writer, and findings are still delivered constructively. "Satisfied" may no longer end the loop before that adversarial pass has run once, and a register finding may be dismissed only with an explicit recorded reason – never silently – while *confidence and silence* is preserved for the objective layers. `redline.md` step 6 (anglicisms and AI-tells) now runs under that adversarial reading. Explicit `--max-iterations=0` is the documented opt-out; `2` / `3` raise the ceiling as before. Scope is the automatic subagent iteration in `/write` and `/edit` only; `/redline`'s human-facing dialogue stays collegial. No "find N findings or justify none" quota is introduced. Audit check (k) pins the new default across the three files so it cannot silently regress. Closes [#42](https://github.com/Kntnt/kntnt-text-skills/issues/42).
...
- **Normalised the repository's British-English prose to `en_GB.md`.** Spaced em-dashes are replaced by the spaced en-dash the rule prescribes (the em-dash is reserved for American English), and serial (Oxford) commas are removed from lists, across every documentation, rule, protocol, genre, technique, skill and metadata file. Excepted: the `lib/languages/` data files (each carries its own language's conventions), the `evals/fixtures/` test inputs (some deliberately hold violations the skills must catch) and the verbatim `LICENSE`. Fixed boilerplate, proper nouns and code are left untouched, and the genre files stay in sync with `_index.md`. The plugin's own documentation now follows the rule it enforces.
...
[Unreleased]: https://github.com/Kntnt/kntnt-text-skills/compare/v0.7.2...HEAD

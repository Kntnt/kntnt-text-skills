...
- **Register dimension tags corrected to match editorial judgement.** Sixteen assertions in `evals/evals.json` carried the wrong dimension because the migration committed raw classifier output without the promised editorial pass: six bare-phrased AI-tell / opening-cliché assertions sat in `mechanics` while their near-identical siblings were `register` (under-counting register and wrongly pooling them into the hard gate); two fast-path short-text restraint checks sat in `register` instead of `protocol`; and nine procedural assertions had dropped to the `mechanics` fallback. `classify_dimensions.py` now encodes the corrected rules, and the committed suite is verified to match its output (a new test fails on any divergence). The `classify` and README claim of a hand-review is corrected to "the suite is kept in sync with the classifier".

...
Inline text: operate on it. Bare invocation with no text or file reference: operate on the most recent text in the conversation — typically the previous assistant message or the text just being worked on. See `protocols/input.md` for file and URL inputs.

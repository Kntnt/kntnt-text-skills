...
High density. Apply the loaded language file's typographic rules rigorously — percentage convention, no-break space between number and unit, dates in the language's standard format. Abbreviations introduced with the full form at first occurrence per the *Abbreviations* section of `rules/constructions.md` — *search engine optimisation (SEO)* — then abbreviated.

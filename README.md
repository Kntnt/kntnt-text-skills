...
- `/proofread` — conservative proofreading. Corrects only what is objectively wrong (spelling, grammar, punctuation, the conventions in the loaded mechanics file). Never changes word choice, word order, structure, tone, or argumentation. The lightest of three review skills: `proofread ⊂ redline ⊂ edit`. Auto-triggers when the user clearly asks for a proofread pass on a specific text (*proofread this*, *fix the typos in this paragraph*, *korrekturläs det här*, …) — not on bare mentions of the word.
...
Each skill's frontmatter `description` defines the trigger boundary. By default a skill activates only on its slash command, the qualified form `/kntnt-text-skills:<skill>`, or natural-language phrasing that names this plugin together with the skill (e.g. *kntnt text skills edit*, *text-edit-skill*, *edit X with Kntnt's skill*). Bare action words like *edit*, *redline*, or *write* do not activate those skills. `/proofread` is the documented exception — it also triggers on clear natural-language proofreading requests aimed at a specific text (*proofread this*, *fix the typos in this paragraph*, *korrekturläs det här*, and equivalents in other languages) because its scope is conservative and broad triggering is safe. Even then, a bare mention of the word *proofread* without a clear request must not activate the skill. `/kntnt-text-skills:help` is a slash command, so it carries the same explicit-invocation property by construction.

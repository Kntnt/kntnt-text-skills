...
Usage (from the project root):
    ...
    uv run evals/workspace/_runner.py
    uv run evals/workspace/_runner.py --limit 4 --parallel 2  # dry-run-ish
    uv run evals/workspace/_runner.py --skip-executors        # only re-grade
    uv run evals/workspace/_runner.py --skill proofread       # one skill
...
class ExecJob:
    ...
    def run_dir(self) -> pathlib.Path:
        """Filesystem directory holding this job's outputs and grading."""
        return (
            WS
            / self.skill
    ...
    def output_md(self) -> pathlib.Path:
        """Path to the executor's delivered output for this job."""
        return self.run_dir / "outputs" / "output.md"

    @property
    def grading_json(self) -> pathlib.Path:
        """Path to the grader's verdict file for this job."""
        return self.run_dir / "grading.json"

    @property
    def done_exec(self) -> bool:
        """True once a non-empty executor output exists — the signal that lets a rerun skip this job."""
        return self.output_md.exists() and self.output_md.stat().st_size > 0

    @property
    def done_grade(self) -> bool:
        """True once a non-empty grading file exists — the resume signal for the grader pass."""
        return self.grading_json.exists() and self.grading_json.stat().st_size > 0

    def label(self) -> str:
        """Human-readable job identifier for progress lines."""
        return (
            f"{self.skill}/eval-{self.case_id:03d}-{self.name}/"
...
def build_job_list(
    ...
    """Expand the suite into one ExecJob per case/config/run, honouring the skill, case-id, and limit filters."""
    jobs: list[ExecJob] = []
...
def render_prompt(cfg_arg: str, case_id: int, iteration: int, run_n: int) -> str:
    """Shell out to _prompts.py to build the executor prompt for one job."""
    cmd = [
        sys.executable,
        str(WS / "_prompts.py"),
        cfg_arg,
...
def run_one_executor(
    job: ExecJob, budget_usd: float, timeout_s: int, model: str
) -> tuple[ExecJob, bool, str]:
    """Run one executor job through `claude -p`, skipping work already on disk. Returns (job, success, status message)."""
    if job.done_exec:
...
def run_executors(
    jobs: list[ExecJob], parallel: int, budget_usd: float, timeout_s: int, model: str
) -> dict[str, int]:
    """Run the executor pass over all jobs with bounded parallelism; return ok/skipped/fail counts."""
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=parallel) as pool:
        futures = {
            pool.submit(run_one_executor, j, budget_usd, timeout_s, model): j
            for j in jobs
        }
...
def grader_batch_prompt(jobs: list[ExecJob]) -> str:
    ...
    lines.append(f"When complete, end your response with: `Graded {len(jobs)} runs.`")
    return "\n".join(lines)


def run_one_grader_batch(
    jobs: list[ExecJob], budget_usd: float, timeout_s: int, model: str
) -> tuple[list[ExecJob], int]:
...
def run_graders(
    ...
    """Grade every executed-but-ungraded job in batches; return graded/missing counts."""
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=parallel) as pool:
        futures = {
            pool.submit(run_one_grader_batch, b, budget_usd, timeout_s, model): b
            for b in batches
        }
        for i, fut in enumerate(concurrent.futures.as_completed(futures), start=1):
            ...
            elapsed = time.time() - t0
            print(
                f"  [batch {i:3d}/{len(batches)}] {graded}/{len(batch)} graded "
                f"(elapsed {elapsed / 60:.1f} min)"
            )
    return stats
...
def main() -> int:
    """Parse CLI args, pre-flight the `claude -p` auth, then run the executor and grader passes."""
    ...
    parser.add_argument(
        "--parallel", type=int, default=4, help="Parallel claude -p invocations"
    )
    parser.add_argument(
        "--executor-budget", type=float, default=1.0, help="Per-executor USD cap"
    )
    parser.add_argument(
        "--grader-budget", type=float, default=3.0, help="Per-grader-batch USD cap"
    )
    ...
    parser.add_argument(
        "--limit", type=int, default=None, help="Cap total executor jobs (debug)"
    )
    parser.add_argument(
        "--skip-executors",
        action="store_true",
    ...
    try:
        ...
        if is_auth_fail:
            print(
                "FAIL — claude -p returned an error. "
    ...
    print(
        f"Job list: {len(jobs)} executor jobs "
        f"({case_count} cases × 2 configs × {args.runs} runs)"
    )

    if not args.skip_executors:
        exec_stats = run_executors(
            jobs, args.parallel, args.executor_budget, args.executor_timeout, args.model
        )
        print(
    ...
    if not args.skip_graders:
        grade_stats = run_graders(
            jobs,
            max(1, args.parallel // 2),

...
Settle each finding via `${CLAUDE_PLUGIN_ROOT}/lib/protocols/dialogue.md`. The user accepts, rejects, counters or delegates.
...
- `--max-iterations=1` / `=2` / `=3`: on delegation, the remaining open findings are handed to `${CLAUDE_PLUGIN_ROOT}/lib/protocols/subagent.md` with that ceiling on iterations. The subagent's convergence rules still apply – early consensus stops the loop. `N > 3` is clamped to 3.
...
- `${CLAUDE_PLUGIN_ROOT}/lib/protocols/dialogue.md` – Phase 3 settling protocol.
...
- `${CLAUDE_PLUGIN_ROOT}/lib/protocols/subagent.md` – loaded speculatively (only used on delegation in Phase 3).
